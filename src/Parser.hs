...
happyExpList = Happy_Data_Array.listArray (0,60) ([61504,49409,49159,0,16384,496,1,4,16,0,0,12288,4096,16508,496,1985,7940,31760,896,49408,1031,31,0,0,0,56,224,896,12288,49152,0,0,0,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parse","Exp","Term","Term_","'('","')'","'+'","'-'","'*'","'/'","sqrt","exp","log","num","real","%eof"]
...
action_4 (5) = happyGoto action_2
action_4 (6) = happyGoto action_3
action_4 _ = happyFail (happyExpListPerState 4)

action_5 (7) = happyShift action_15
...
action_24 (8) = happyShift action_27
...
happyReduction_8 (HappyTerminal (REAL happy_var_1))
	 =  HappyAbsSyn6
		 (happy_var_1
	)
...
happyReduction_10 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn6
		 (fromIntegral (round (sqrt happy_var_3 * 10^7)) / 10^7
	) `HappyStk` happyRest
...
happyReduction_11 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn6
		 (fromIntegral (round (exp happy_var_3 * 10^7)) / 10^7
	) `HappyStk` happyRest
...
happyReduction_12 (_ `HappyStk`
	...
	happyRest)
	 = HappyAbsSyn6
		 (fromIntegral (round (log happy_var_3 * 10^7)) / 10^7
	) `HappyStk` happyRest
...
happyNewToken action sts stk (tk:tks) =
	...
	REAL happy_dollar_dollar -> cont 17;
	_ -> happyError' ((tk:tks), [])
	}
